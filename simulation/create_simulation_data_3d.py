...
for i in range(2000):
	...
	response = np.sum(template[4:8, 14:18, 5]) - np.sum(template[4:8, 14:18, 7])
	template = template.reshape(-1)
	template = np.append(template, response)

	data[i] = template

noise_col = rng.normal(loc=0.0, scale=1.0, size=(1, 2000))

data[:, -1] = data[:, -1] + noise_col

np.savetxt("simulation_data_lagged_rf.csv", data, delimiter=",")
